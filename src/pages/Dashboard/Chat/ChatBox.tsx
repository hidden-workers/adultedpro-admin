...
export const ChatBox = () => {
  ...
  useEffect(() => {
    ...
    let jobId;
    if (selectedChat) {
      // otherUser = getOtherUserDetail(selectedChat.participants, mongoUserId);
      if (
        typeof selectedChat.jobId === 'object' &&
        selectedChat.jobId !== null
      ) {
        jobId = selectedChat.jobId._id;
      } else {
  ...
  return (
    <>
      ...
      {!selectedChat?.id ? (
        ...
        <div className="flex h-full flex-col border-l border-stroke dark:border-strokedark  ">
          {/* <!-- ====== Chat Box Start --> */}
              ...
              (selectedChat?.isUserApplication ||
                ...
                <div className="flex gap-1">
                  <button
                    onClick={() => onReject()}
                    disabled={
                      findedUserApplication?.status ==
                      UserApplicationStatus?.Disqualified
                    }
                    className="w-max-content flex h-fit items-center justify-center rounded-md bg-red px-4 py-2.5 text-white hover:bg-opacity-90 disabled:cursor-not-allowed disabled:bg-red/75"
                  >
                    Reject
                  </button>
                  <button
                    ...
                    className="w-max-content flex h-fit items-center justify-center rounded-md bg-primary px-4 py-2.5 text-white hover:bg-opacity-90 disabled:cursor-not-allowed disabled:bg-primary/75"
                  >
                    Invite to interview
                  </button>
                </div>
          ...
          <div
            ref={scrollRef}
            className="n-scrollbar h-full max-h-full space-y-3.5 overflow-y-auto px-6 py-7.5 "
          >
            {loadingMessages
              ? Array(5)
                  .fill('')
