...
const CandidateDetail = ({ filterType }: { filterType: 'all' | 'other' }) => {
  ///////////////////////////////////////////////////// VARIABLES ///////////////////////////////////////////////////
  ...
  const onBookmarkApplication = (userApplication: UserApplication) => {
    ...
    if (isBookmarked) {
      ...
      dispatch<any>(
        ...
        // setMainBranch(() => ({ ...employer, bookmarkedUserApplications }));
  ...
  const onChatWithApplicant = (userApplication: UserApplication) => {
    ...
    if (selectedApplicantChat.length > 0) {
      ...
      const newChatData: Chat = {
        participants: [mongoUserId, userApplication?.applicant?.id], // Assuming chat participants are stored in an array
        lastMessage: '',
  ...
  const onChatWithStudent = (student: User) => {
    ...
    const selectedApplicantChat = userChats?.filter((chat) =>
      chat?.participants?.some(
        ...
        (participant) => participant?._id === student?.id,
      ),
    );
    ...
    if (selectedApplicantChat?.length > 0) {
      ...
      const newChatData: Chat = {
        participants: [mongoUserId, student?.id],
        lastMessage: '',
        lastMessageTimestamp: new Date(),
        jobId: '',
        role,
