...
const CreateTodo: React.FC<Props> = ({ open, setOpen, todo }) => {
  ...
  const onCreate = () => {
    ...
    if (todoType === 'personal') {
      ...
      Promise.all(
        ids?.map((id) => {
          dispatch<any>(createTodo({ ...formData, userId: id, dueDateTime }));
        }),
      )
        .then(() => {
          ...
          {todoType === 'program' && (
            <div className="flex flex-col items-start w-full">
              ...
              <div className="relative w-full">
                <button
                  type="button"
                  className="w-full text-left rounded border-[1.5px] border-stroke bg-transparent px-5 py-3 font-normal text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary"
                ...
                {dropdownOpen && (
                  <div className="absolute mt-2 bg-white border border-gray-200 rounded-md shadow-lg dark:bg-gray-800 dark:border-gray-600 w-full max-h-40 overflow-y-auto z-10">
                      ...
                      {programs.map((program) => (
                        <label key={program.id} className="flex items-center">
                          <input
                            ...
                            onChange={() => onOptionChange(program.id)}
                            className="mr-2"
                          />
                          {program.name}
                        </label>
